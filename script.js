        ...
        if (def && def.type === 'glb') {
            ...
            const _glbOpaqueShader = new Cesium.CustomShader({
                ...
                fragmentShaderText: [
                    ...
                    '    // Window glass = m0mat_007, the only material with baseColorFactor',
                    '    // alpha < 1 (0.25). The old shader forced alpha=1 everywhere, making',
                    '    // glass render as solid black. Discard glass fragments instead so',
                    '    // windows are fully see-through. discard works in OPAQUE mode and',
                    '    // avoids the per-primitive depth-sort bug that plagued BLEND mode.',
