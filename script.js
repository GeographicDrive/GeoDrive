...
const VEHICLE_MODEL_DEFS = {
    ...
    //
    // ROTATION FIX — actually opened car.glb and walked its node hierarchy
    // (full world-space transforms, including the Sketchfab root correction
    // matrix) to find the real nose direction: the "Wing0a" (rear spoiler)
    // mesh and dual exhaust-tip geometry sit at one extreme of the model's Z
    // range, the grille meshes + front fascia detailing sit at the other.
    // Conclusion: the asset is genuinely authored Y-up / +Z-forward —
    // textbook standard glTF convention, no oddities in the source data.
    //
    // And yet upAxis:'Y'/forwardAxis:'Z' (textbook-correct, tried first)
    // rendered the car sideways, and upAxis:'Y'/forwardAxis:'X' (tried
    // next) rendered it 180° backward. Both results are still useful data:
    // a "sideways" symptom is consistent with either +90° or −90° of
    // error (visually indistinguishable on a roughly symmetric car), and a
    // "facing exactly backward" symptom is unambiguous (180°, no sign
    // ambiguity). Solving for the one forward axis consistent with BOTH
    // observations points to forwardAxis: 'NEGATIVE_X' — not the
    // geometrically "correct" Z axis the file actually uses.
    //
    // That mismatch between a model's real authored axis and the axis
    // value CesiumJS actually needs to render it correctly is a known,
    // independently-documented quirk (CesiumJS forum: a user reported that
    // for glTF 2.0 models — which are spec'd Y-up/Z-forward — the only
    // setting that renders correctly in their Cesium version is Y-up/
    // X-forward, not Y-up/Z-forward; ours lands on NEGATIVE_X, the mirror
    // of their X, which fits the same family of behavior on possibly a
    // different Cesium version/build). Rather than fight that quirk
    // theoretically, NEGATIVE_X is what the car's own two real render
    // results converge on.
    car: {
        type: 'glb',
        url: 'car.glb',
