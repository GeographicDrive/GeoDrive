...
const _glbOpaqueShaderPlane = new Cesium.CustomShader({
    ...
    fragmentShaderText: [
        ...
        '    material.diffuse = material.diffuse + vec3(1.0, 0.55, 0.15) * warmth * 0.45;',
        '',
        '    // ── Night: strobe flash (brief white pulse every 1.5 s) ──────',
        '    float strobePhase = mod(u_time, 1.5);',
        '    float strobe = u_nightMode * (strobePhase < 0.08 ? 1.0 : 0.0);',
        '    material.diffuse = material.diffuse + vec3(1.0) * strobe * 0.35;',
        '}'
    ].join('\n')
});
