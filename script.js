...
let camSmoothPitch   = null;   // number (radians) | null — plane 3rd-person: lagged pitch attitude
let camSmoothRoll    = null;   // number (radians) | null — plane 3rd-person: lagged roll attitude
...
function updateCesiumCamera(dt) {
    ...
    if (settings.cesiumCameraMode === 'third') {
        ...
        if (isPlane) {
            // ── Plane: per-axis inertia (altitude, pitch, roll, heading) ───────
            //
            // Each axis has its own spring rate, all divided by followDelay so
            // the Settings slider stretches or compresses the whole system:
            //   height  — fast  : catches up altitude changes quickly
            //   pitch   — medium: nose-up/down lags with a cinematic sweep
            //   roll    — slow  : banking hangs the longest, most "weighty"
            //
            // The camera POSITION is computed from a transform that includes
            // the lagged pitch and roll so the "behind the aircraft" offset
            // vector orbits around the aircraft with inertia instead of
            // snapping immediately to the new attitude.
            const heightAlpha = 1.0 - Math.exp(-(8 / followDelay) * dt);  // altitude spring
            const pitchAlpha  = 1.0 - Math.exp(-(4 / followDelay) * dt);  // pitch spring (slower)
            const rollAlpha   = 1.0 - Math.exp(-(3 / followDelay) * dt);  // roll spring (slowest)
            ...
            // ── Pitch lag ─────────────────────────────────────────────────────
            // Smooths flight.pitch so the camera offset frame tilts toward the
            // new nose attitude gradually, not instantly.
            if (camSmoothPitch === null) {
                camSmoothPitch = flight.pitch;
            } else {
                camSmoothPitch += (flight.pitch - camSmoothPitch) * pitchAlpha;
            }

            // ── Roll lag ──────────────────────────────────────────────────────
            // Smooths flight.roll. Used both to tilt the camera position offset
            // (small position effect) and to tilt the camera's "up" vector so
            // the horizon banks like a real flight simulator.
            if (camSmoothRoll === null) {
                camSmoothRoll = flight.roll;
            } else {
                camSmoothRoll += (flight.roll - camSmoothRoll) * rollAlpha;
            }

            // ── Camera POSITION ───────────────────────────────────────────────
            // Build the local frame at the aircraft's REAL lat/lng but LAGGED
            // altitude, and orient it with the lagged heading + pitch + roll so
            // the (0, -back, up) offset sweeps with inertia as the aircraft
            // changes attitude.
            //
            // Note: for the offset frame we use flight.pitch / flight.roll in
            // their natural aviation slots (unlike vehicleHpr which must swap
            // them to correct Cesium's box-model axis mis-match — that swap is
            // only needed for making the visible model look right, not here).
            const camAnchorPos = Cesium.Cartesian3.fromDegrees(state.lng, state.lat, camSmoothHeight);
            const smoothHpr = new Cesium.HeadingPitchRoll(camSmoothHeading, camSmoothPitch, camSmoothRoll);
        ...
        // For the plane in 3rd person, blend the camera's up-vector toward
        // the aircraft's LAGGED up-direction so the horizon tilts with roll
        // and recovers with inertia — the classic flight-simulator banking feel.
        // We re-use the vehicleHpr axis-swap convention (pitch↔roll slot swap,
        // roll negated) because we want the "local Z" extracted from this
        // transform to match the visual model's actual up axis.
        let cameraUp = enuUp;
        if (isPlane && camSmoothRoll !== null && camSmoothPitch !== null) {
            const laggedHpr = new Cesium.HeadingPitchRoll(
                Cesium.Math.toRadians(state.heading),
                -camSmoothRoll,   // HPR "pitch" slot ← -roll  (vehicleHpr convention)
                 camSmoothPitch   // HPR "roll"  slot ←  pitch
            );
            const laggedT  = Cesium.Transforms.headingPitchRollToFixedFrame(vehiclePosition, laggedHpr);
            const laggedUp = Cesium.Matrix4.multiplyByPointAsVector(
                laggedT, new Cesium.Cartesian3(0, 0, 1), new Cesium.Cartesian3()
            );
            Cesium.Cartesian3.normalize(laggedUp, laggedUp);
            cameraUp = laggedUp;
        }
