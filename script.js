...
function initNavMap() {
    ...
    // Make the overlay visible now that Leaflet is ready.
...
function _removeCinematicColorGrade() {
    if (_cinematicColorGradeStage && cesiumViewer) {
        try { cesiumViewer.scene.postProcessStages.remove(_cinematicColorGradeStage); } catch(e) {}
...
function _addCinematicColorGrade() {
    ...
    const frag = `
        ...
        void main() {
            ...
            // 3. Smooth S-curve contrast (smoothstep-based)
            vec3 sc = col * col * (3.0 - 2.0 * col);
            col = mix(col, sc, 0.38);

            // 4. Golden-hour warm tint
            col.r = min(col.r * 1.07, 1.0);
            col.g = min(col.g * 1.01, 1.0);
            col.b = col.b * 0.93;

            // 5. Vignette — gentle, only noticeable at edges
            vec2 uv  = v_textureCoordinates;
            vec2 off = (uv - 0.5) * 1.6;
            float vig = 1.0 - clamp(dot(off, off), 0.0, 1.0) * 0.50;
            col *= vig;
...
function _applyCinematicCesium() {
    ...
    sc.skyAtmosphere.saturationShift =  0.42;
    sc.skyAtmosphere.brightnessShift =  0.18;

    // ── Globe / terrain atmosphere ─────────────────────────────────────────
    sc.globe.enableLighting              = true;
    sc.globe.atmosphereHueShift          = -0.04;
    sc.globe.atmosphereSaturationShift   =  0.25 + contrast * 0.45;
    sc.globe.atmosphereBrightnessShift   =  0.08;
    try { sc.globe.atmosphereLightIntensity = 8 + contrast * 18; } catch(e) {}
