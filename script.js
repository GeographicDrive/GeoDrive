 ...
 * a given lng/lat.
 *
 * Supports both Cesium WorldTerrain (via globe.getHeight) AND Google
 * Photorealistic 3D Tiles (via scene.pickPosition raycast).
 *
 * When GP3DT is active, we cast a ray from above the vehicle down to the
 * ground using the scene's GPU picking. We temporarily hide the vehicle
 * primitives to avoid picking the vehicle itself.
 *
 * Falls back to globe.getHeight() when:
 *   - No photorealTileset is loaded (WorldTerrain / Ellipsoid mode)
 *   - The vehicle is off-screen or windowPos is null (e.g. during a glitch)
 *   - The GPU pick doesn't hit the GP3DT mesh (e.g. first frames, tile gap)
...
function getGroundHeight(lng, lat) {
    ...
    // 1. 尝试从 Photorealistic 3D Tiles 获取高度
    if (photorealTileset) {
        // 从高空开始射线检测，确保能穿过 GP3DT
        const position = Cesium.Cartesian3.fromDegrees(lng, lat, 10000);
        const windowPos = Cesium.SceneTransforms.wgs84ToWindowCoordinates(cesiumViewer.scene, position);

        if (windowPos) {
            // 临时隐藏车辆模型，避免拾取到车辆自身
            const wasVisible = {};
            Object.values(vehiclePrimitives).forEach(parts => parts.forEach(p => {
                wasVisible[p] = p.show;
                p.show = false;
            }));

            // 拾取屏幕位置
            const pickedObject = cesiumViewer.scene.pick(windowPos);
            let pickedHeight = null;

            // 确保拾取到的是 GP3DT
            if (pickedObject && pickedObject.primitive === photorealTileset) {
                const pickedPos = cesiumViewer.scene.pickPosition(windowPos);
                if (pickedPos) {
                    const cartographic = Cesium.Cartographic.fromCartesian(pickedPos);
    ...
    // 2. Fallback 到 Globe terrain (WorldTerrain 或 Ellipsoid)
