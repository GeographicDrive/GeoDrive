...
function _applyDayNightMode() {
    ...
    if (isNightMode) {
        ...
        // MIX blends the baked colour toward a near-black tint (amount 0.78
        // → ~78% of the dark tint, 22% of the original colour) giving a
        ...
        if (photorealTileset) {
            ...
            photorealTileset.colorBlendAmount = 0.78;
...
const _glbOpaqueShaderPlane = new Cesium.CustomShader({
    ...
    fragmentShaderText: [
        ...
        '',
        '    // ── Night: base body glow so plane is not pitch-black ────────',
        '    // material.emissive is additive and scene-light-independent,',
        '    // which is what we need when DirectionalLight intensity = 0.05.',
        '    material.emissive = material.emissive + vec3(0.03, 0.03, 0.07) * u_nightMode;',
        '',
        '    // ── Night: cockpit instrument glow ───────────────────────────',
        '    // Use emissive (not diffuse) so cockpit panels glow in 1st-person',
        '    // even though the scene has barely any ambient light at night.',
        '    float warmth = u_nightMode * u_cockpitAmbient;',
        '    material.emissive = material.emissive + vec3(1.0, 0.55, 0.15) * warmth * 0.40;',
        '    material.diffuse  = material.diffuse  + vec3(1.0, 0.55, 0.15) * warmth * 0.10;',
        '',
        '    // ── Night: anti-collision strobe (subtle pulse on fuselage) ──',
        '    // Using emissive instead of diffuse keeps the flash from making',
        '    // the whole plane look like a camera flash went off.',
        '    float strobePhase = mod(u_time, 1.5);',
        '    float strobe = u_nightMode * (strobePhase < 0.08 ? 1.0 : 0.0);',
        '    material.emissive = material.emissive + vec3(1.0) * strobe * 0.12;',
        '}'
    ].join('\n')
});
