        ...
        const shaderOptions = {
            ...
            fragmentShaderText: `
                void fragmentMain(FragmentInput fsInput, inout czm_modelMaterial material) {
                    ...
                    // ── Roof-only projection ──────────────────────────────────────
                    // The local "up" direction at any point on Earth's surface is
                    // simply normalize(positionWC) — the outward ECEF radial vector.
                    // A flat roof has a surface normal nearly parallel to this vector
                    // (dot product close to 1.0). Vertical walls have normals nearly
                    // perpendicular to it (dot product near 0). We only project the
                    // satellite texture onto faces where the surface is upward-facing
                    // enough (roofness > 0.7 ≈ within ~45° of horizontal), leaving
                    // walls with their default OSM Buildings colour untouched.
                    vec3 worldUp = normalize(p);
                    // fsInput.attributes.normalEC is in eye (camera) space.
                    // czm_inverseViewRotation (mat3) rotates eye → world, giving us
                    // the face normal in the same ECEF frame as worldUp.
                    vec3 normalWC = normalize(czm_inverseViewRotation * fsInput.attributes.normalEC);
                    float roofness = dot(normalWC, worldUp);
                    // Not a roof — leave material unchanged so walls keep their
                    // default OSM Buildings white/grey appearance.
                    if (roofness < 0.7) return;
