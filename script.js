...
function _applyDayNightMode() {
    ...
    if (isNightMode) {
        ...
        // Spawn flashing runway & plane lights after a short delay
        // so the scene is ready
        setTimeout(_spawnNightLights, 400);
...
/** Spawn flashing lights for the runway nearest the player and for the plane */
function _spawnNightLights() {
    if (!cesiumViewer || !isNightMode) return;
    _clearNightRunwayLights();

    const lat = state.lat, lng = state.lng;
    const groundH = getGroundHeight(lng, lat);

    // ── Runway edge lights: two rows of white lights each side of runway ─
    // We approximate the runway as a 2.5 km strip in the player heading direction
    const hdgRad = state.heading * Math.PI / 180;
    const RUNWAY_LENGTH_M = 2500;
    const RUNWAY_HALF_WIDTH_M = 22.5;
    const NUM_LIGHTS = 20;
    const SPACING    = RUNWAY_LENGTH_M / NUM_LIGHTS;

    for (let i = 0; i <= NUM_LIGHTS; i++) {
        const fwd   = (i - NUM_LIGHTS / 2) * SPACING;
        const fwdLat = fwd  * Math.cos(hdgRad) / 111320;
        const fwdLng = fwd  * Math.sin(hdgRad) / (111320 * Math.cos(lat * Math.PI / 180));
        const sideM  = RUNWAY_HALF_WIDTH_M;
        const sRad   = hdgRad + Math.PI / 2;
        const sideLat = sideM * Math.cos(sRad) / 111320;
        const sideLng = sideM * Math.sin(sRad) / (111320 * Math.cos(lat * Math.PI / 180));

        // Threshold end = red/green; rest = white
        const isThreshold = (i === 0 || i === NUM_LIGHTS);
        const edgeCol  = isThreshold ? Cesium.Color.RED.withAlpha(0.9) : Cesium.Color.WHITE.withAlpha(0.85);
        const edgeColG = isThreshold ? Cesium.Color.GREEN.withAlpha(0.9) : Cesium.Color.WHITE.withAlpha(0.85);

        // Left edge
        _nightRunwayLights.push(cesiumViewer.entities.add({
            position: Cesium.Cartesian3.fromDegrees(lng + fwdLng - sideLng, lat + fwdLat - sideLat, groundH + 0.3),
            point: { pixelSize: 5, color: edgeCol, outlineColor: Cesium.Color.YELLOW.withAlpha(0.3), outlineWidth: 1, scaleByDistance: new Cesium.NearFarScalar(100, 2.5, 3000, 0.8) }
        }));
        // Right edge
        _nightRunwayLights.push(cesiumViewer.entities.add({
            position: Cesium.Cartesian3.fromDegrees(lng + fwdLng + sideLng, lat + fwdLat + sideLat, groundH + 0.3),
            point: { pixelSize: 5, color: edgeColG, outlineColor: Cesium.Color.YELLOW.withAlpha(0.3), outlineWidth: 1, scaleByDistance: new Cesium.NearFarScalar(100, 2.5, 3000, 0.8) }
        }));
    }
    ...
    for (let i = 1; i < NUM_LIGHTS; i++) {
        ...
        const fwdLng = fwd * Math.sin(hdgRad) / (111320 * Math.cos(lat * Math.PI / 180));
        _nightRunwayLights.push(cesiumViewer.entities.add({
            position: Cesium.Cartesian3.fromDegrees(lng + fwdLng, lat + fwdLat, groundH + 0.15),
            point: { pixelSize: 3, color: Cesium.Color.WHITE.withAlpha(0.7), scaleByDistance: new Cesium.NearFarScalar(50, 2, 2000, 0.5) }
        }));
    }

    // ── Approach PAPI/VASI: 4 red lights on the left side of threshold ───
    for (let p = 0; p < 4; p++) {
        const fwdPapi = -RUNWAY_LENGTH_M / 2 - 30;
        const fwdLat  = fwdPapi  * Math.cos(hdgRad) / 111320;
        const fwdLng  = fwdPapi  * Math.sin(hdgRad) / (111320 * Math.cos(lat * Math.PI / 180));
        const sRad    = hdgRad + Math.PI / 2;
        const offLat  = (RUNWAY_HALF_WIDTH_M + 10 + p * 5) * Math.cos(sRad) / 111320;
        const offLng  = (RUNWAY_HALF_WIDTH_M + 10 + p * 5) * Math.sin(sRad) / (111320 * Math.cos(lat * Math.PI / 180));
        _nightRunwayLights.push(cesiumViewer.entities.add({
            position: Cesium.Cartesian3.fromDegrees(lng + fwdLng + offLng, lat + fwdLat + offLat, groundH + 1.2),
    ...
    console.log(`[Night] Spawned ${_nightRunwayLights.length} runway lights`);
}
...
const _glbOpaqueShaderPlane = new Cesium.CustomShader({
    ...
    fragmentShaderText: [
        ...
        '    material.alpha = 1.0;',
        '',
        '    // ── Enhanced metallic/specular response ─────────────────────',
        '    // Boost specular on metallic parts (fuselage, wing skin)',
        '    // and apply a mild base ambient so the underside is not pitch black.',
        '    vec3 viewDir = normalize(czm_viewerPositionWC - fsInput.attributes.positionWC);',
        '',
        '    // Detect metallic surfaces (high metalness, light color)',
        '    float isMetal = step(0.4, material.metallic);',
        '    float brightness = dot(material.diffuse, vec3(0.299, 0.587, 0.114));',
        '',
        '    // Ambient fill: prevents pure black underside in all lighting',
        '    float ambientStrength = mix(0.18, 0.08, isMetal);',
        '    material.diffuse += material.diffuse * ambientStrength;',
        '',
        '    // Specular edge highlight (Fresnel-like rim lighting)',
        '    float NdotV = max(0.0, dot(material.normalEC, vec3(0.0, 0.0, 1.0)));',
        '    float rim = pow(1.0 - NdotV, 3.0);',
        '    vec3 rimColor = mix(vec3(0.3, 0.35, 0.5), vec3(1.0, 0.95, 0.85), brightness);',
        '    material.diffuse += rim * rimColor * 0.25 * isMetal;',
        '',
        '    // ── Night mode: cockpit warm glow ───────────────────────────',
        '    // Add a warm orange-amber light to interior surfaces when night mode is on.',
        '    float cockpitWarmth = u_nightMode * u_cockpitAmbient;',
        '    vec3  warmLight     = vec3(1.0, 0.55, 0.15) * cockpitWarmth * 0.6;',
        '    material.diffuse += warmLight * (1.0 - isMetal);',
        '',
        '    // Strobe highlight on fuselage belly in night mode (very brief flash)',
        '    float strobePhase = mod(u_time, 1.5);',
        '    float strobe = u_nightMode * step(0.0, strobePhase) * step(strobePhase, 0.08);',
        '    material.diffuse += vec3(1.0, 1.0, 1.0) * strobe * 0.4 * (1.0 - isMetal);',
        '}'
    ].join('\n')
});
...
function update() {
    ...
    if (settings.renderMode === '3D' && cesiumViewer) {
        ...
        if (isNightMode && state.vehicle === 'plane') {
            ...
            // Add/update a warm cockpit glow billboard visible in 3rd person
            if (!_cockpitLightSource && cesiumViewer) {
                const ccamCk = activeCockpitCam();
                _cockpitLightSource = cesiumViewer.entities.add({
                    position: new Cesium.CallbackProperty(() => {
                        const h = (flight.groundRef || 0) + flight.alt * 0.3048;
                        const anchorPos = Cesium.Cartesian3.fromDegrees(state.lng, state.lat, h);
                        const hpr = new Cesium.HeadingPitchRoll(Cesium.Math.toRadians(state.heading), -flight.roll, flight.pitch);
                        const xf  = Cesium.Transforms.headingPitchRollToFixedFrame(anchorPos, hpr);
                        const off = new Cesium.Cartesian3(ccamCk.sidePosition, ccamCk.frontBack, ccamCk.upDown - 0.5);
                        return Cesium.Matrix4.multiplyByPoint(xf, off, new Cesium.Cartesian3());
                    }, false),
                    point: {
                        pixelSize: 14,
                        color: new Cesium.Color(1.0, 0.55, 0.15, 0.45),
                        outlineColor: new Cesium.Color(1.0, 0.7, 0.3, 0.2),
                        outlineWidth: 6,
                        scaleByDistance: new Cesium.NearFarScalar(2, 3.5, 200, 0.4)
                    }
                });
            }
        } else if (!isNightMode && _cockpitLightSource && cesiumViewer) {
            cesiumViewer.entities.remove(_cockpitLightSource);
            _cockpitLightSource = null;
        }
        // Update shader uniforms every frame so night/cockpit glow animates
        if (_glbOpaqueShaderPlane && _glbOpaqueShaderPlane.uniforms) {
            ...
            _glbOpaqueShaderPlane.uniforms.u_cockpitAmbient.value = (settings.cesiumCameraMode === 'first') ? 1.0 : 0.3;
        }
...
const RunwayDB = (() => {
    ...
    return { load, getRunways, isLoaded };
})();
