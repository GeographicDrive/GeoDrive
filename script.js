...
/**
 * setGoogleTilesStatus — writes a message into the #google-tiles-status
 * line in the Settings panel (hidden when message is empty). This is the
 * fix for failures being completely silent: every success AND every
 * failure path in tryLoadGoogleTiles() now calls this, so something always
 * shows up next to the textbox the key was typed into — not just in the
 * console.
 */
function setGoogleTilesStatus(message, type) {
    const el = document.getElementById('google-tiles-status');
    if (!el) return;
...
/**
 * diagnoseGoogleTilesError — turns whatever Cesium rejects with into one of
 * a few specific, actionable messages instead of a generic "failed to
 * load". Cesium's tileset request failures normally surface as a
 * RequestErrorEvent with a `statusCode`; older builds sometimes only put
 * the code in `.message`, so both are checked.
 */
function diagnoseGoogleTilesError(e) {
    const statusCode = e && (e.statusCode || (e.response && e.response.status));
    const msg = (e && e.message) || String(e || '');

    if (statusCode === 403 || /\b403\b/.test(msg)) {
        return '403 Forbidden — enable the "Map Tiles API" (not just "Maps JavaScript API") on this key\'s Google Cloud project, and if the key has HTTP referrer restrictions, add this exact domain to the allow-list.';
    }
    if (statusCode === 404 || /\b404\b/.test(msg)) {
        return '404 Not Found — this project likely has no active billing account. The Map Tiles API requires real billing on file even within the free quota.';
    }
    if (statusCode === 400 || /API key not valid/i.test(msg)) {
        return '400 Bad Request — the key looks invalid or malformed. Re-copy it from the Google Cloud console.';
    }
    if (statusCode === 429 || /\b429\b/.test(msg)) {
        return '429 Too Many Requests — quota or rate limit hit on this key/project.';
    }
...
function tryLoadGoogleTiles() {
    ...
    setGoogleTilesStatus('Loading photorealistic 3D tiles…', 'info');
    try {
        ...
        Promise.resolve(tilesetPromise)
            .then(tileset => {
                ...
                setGoogleTilesStatus('✓ Photorealistic 3D tiles loaded.', 'success');
            })
            .catch(e => {
                console.warn('Google Photorealistic 3D Tiles failed to load, staying on ArcGIS imagery:', e);
                setGoogleTilesStatus(diagnoseGoogleTilesError(e), 'error');
            });
    } catch (e) {
...
function applyGoogleTilesKey() {
    ...
    if (!cesiumViewer) {
        ...
        setGoogleTilesStatus('Key saved, but the 3D engine isn\'t running in this browser (it fell back to CSS mode), so tiles can\'t load yet.', 'error');
        return;
    }
    tryLoadGoogleTiles();
}
...
function updateCesiumCamera(dt) {
    ...
    if (settings.cesiumCameraMode === 'third') {
        ...
        // Rather than snapping the camera rigidly to vehicleTransform every
        // frame (which feels "glued on"), we:
        //  1. Compute the TARGET camera world position from a smoothed
        //     heading that lags behind the vehicle's real heading.
        //  2. Lerp the actual camera world position toward that target.
        // The result is a floaty, cinematic chase cam that sweeps around
        // corners instead of snapping.
        ...
        // How quickly the smoothed heading and position chase the vehicle.
        // Lower = more lag (dreamier); higher = snappier.  dt-scaled so
        // frame-rate changes don't affect the feel. settings.cameraFollowDelay
        ...
        const headingAlpha  = 1.0 - Math.exp(-(6 / followDelay) * dt);   // heading spring
        ...
        // ── 2. Compute TARGET camera position from smoothed heading ───────
        // Build a local ENU frame using the smoothed heading, then place the
        // camera back/up from the vehicle in that frame.
        ...
        // ── 3. Lerp actual camera position toward the target ──────────────
        ...
        // ── 4. Point camera from its smoothed position toward the vehicle ─
        // Use setView so we control position + direction explicitly.
        cesiumViewer.camera.lookAtTransform(Cesium.Matrix4.IDENTITY);
        const dir = Cesium.Cartesian3.subtract(vehiclePosition, camSmoothPos, new Cesium.Cartesian3());
