...
function updateFlight(dt) {
    ...
    // elevator/aileron authority is locked out, exactly like a real aircraft
    // before rotation. The rudder is the only way to steer until then
    // (nosewheel/rudder steering on the runway).
    const canPitchRoll = state.speed >= ROTATION_SPEED_KTS || flight.alt > 25;
    ...
    // While still on the ground and below rotation speed, use direct
    // nosewheel/rudder steering on state.heading (the aero rudder moment is
    // negligible at taxi speed) and keep the sim's yaw in sync with it.
    if (!canPitchRoll) {
        if (Math.abs(yawInput) > 0.01) {
            state.heading = (state.heading + yawInput * 30 * dt + 360) % 360;
        }
        sim.setAttitude(state.heading, 0, 0);
        sim.state[3] = sim.state[4] = sim.state[5] = 0; // zero body rates while taxiing
    }
    ...
    flight.pitch = Math.max(-1.3, Math.min(1.3, euler.pitch));
    flight.roll  = Math.max(-1.5, Math.min(1.5, euler.roll));
