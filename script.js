...
const settings = {
    ...
    mapStyle: 'photoreal' // '3D Map Style' toggle in Settings — 'cesium' (WorldTerrain + OSM Buildings) or 'photoreal' (Google Photorealistic 3D Tiles). Gates tryEnableWorldTerrain/tryLoadOsmBuildings/tryLoadPhotorealisticTiles — see set3DStyle().
};
...
document.getElementById('vehicle-select').addEventListener('change', e => {
    ...
    if (state.vehicle === 'plane') {
        ...
        flight.flaps = 0; flight.gearDown = true; flight.brakeActive = true;
...
const flight = {
    ...
    gearDown: true, brakeActive: true,
...
function updateFlight(dt) {
    ...
    // ── REALISTIC THRUST & DRAG MODEL ──────────────────────────────────────
    const maxThrust = 120; // max 120 knots of thrust at full throttle
    const thrust = (flight.throttle / 100) * maxThrust;

    const dragCoeff = 0.012;
    const inducedDrag = state.speed * state.speed * dragCoeff;
    const parasiteDrag = (flight.gearDown ? 10 : 0) + (flight.flaps * 5);
    const brakeDrag = flight.brakeActive ? 60 : 0;

    const netForce = thrust - inducedDrag - parasiteDrag - brakeDrag;
    state.speed += netForce * 0.04 * dt;
    state.speed = Math.max(0, state.speed);

    // ── STALL MODEL ────────────────────────────────────────────────────────
    const stallSpeed = 50 - (flight.flaps * 8);
