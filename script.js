...
function buildVehicleModels() {
    ...
    Object.keys(VEHICLE_MODEL_DEFS).forEach(type => {
        ...
        if (def && def.type === 'glb') {
            ...
            // ── TRANSPARENCY FIX ─────────────────────────────────────────
            // The a320.glb has materials with alphaMode:"BLEND", which puts
            // them in Cesium's translucent render pipeline.  That pipeline
            // sorts geometry by depth PER-PRIMITIVE (not per-triangle), so
            // parts of the fuselage behind the landing gear end up rendered
            // on top of it — exactly the "see-through" look in the cockpit.
            //
            // CustomShader is Cesium's official public API to override
            // fragment outputs after the normal PBR evaluation.  Setting
            // material.alpha = 1.0 here forces every fragment to full
            // opacity regardless of what alphaMode the glTF declares.
            // The model stays textured / lit normally; only the alpha
            // channel is clamped to 1.  No washed-out colours, no colour
            // property needed on the Model itself (which activates the
            // colour-blend pipeline and DID cause the translucency before).
            ...
            Cesium.Model.fromGltfAsync({
                ...
                // Forces every mesh fragment to alpha=1 via the custom
                // fragment shader above, overriding any BLEND alpha mode
                // declared in the glTF material — this is what eliminates
                // the see-through fuselage without touching model.color.
